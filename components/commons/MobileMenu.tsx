...
const MobileMenu = () => {
    ...
    return (
        <div className='md:hidden fixed z-40 right-3 top-3'>
            <div
                onClick={toggleMenu} 
                className='relative w-[12vw] border border-[#87807040] rounded-md right-3 top-3 z-40 bg-[#ffffffe8] flex flex-col items-center gap-[5px] py-4'
            >
            ...
            <ul className={`${isToggle ? 'w-[90vw] h-[75vh] top-0 right-0' : 'w-0 h-0 -top-[200%] right-[50%]'} absolute overflow-hidden p-8 flex flex-col justify-center gap-5 bg-[#ffffffe8] duration-700`}>
                ...
                <Link href={'/#products'} className='border-b p-2 text-2xl' onMouseUp={toggleMenu}>
                    Products
                </Link>
